...
app.post("/reset-password", async (req, res) => {
  ...
  if (!stored) return res.status(400).json({ status: "No OTP requested for this email" });
  if (Date.now() > stored.expires) {
    delete otpStore[email];
    return res.status(400).json({ status: "OTP expired. Request a new one." });
  }
  if (stored.otp !== otp) return res.status(400).json({ status: "Invalid OTP" });
...
app.post("/api/google/add-event", async (req, res) => {
  try {
    ...
    // Parse time - extract hours or default to 10:00
    let startHour = 10;
    if (event.time) {
...
// --- ORCHESTRATOR ENDPOINT (MCP-style) ---
app.post('/api/orchestrate', userMiddleware, async (req, res) => {
